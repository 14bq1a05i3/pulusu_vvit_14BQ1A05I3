...
#For testing output file change location
# itemMasterOutboundPath = '/mnt/centralized-price-promo/MVPTestinQA'
...
# MVP Changes - 2022-02-09
def salePriceMVP(SCRTX_DET_FREQ_SHOP_TYPE, SCRTX_DET_FREQ_SHOP_VAL, SMA_RETL_MULT_UNIT, SMA_MULT_UNIT_RETL, SCRTX_HDR_DESC):
    if ((SCRTX_DET_FREQ_SHOP_TYPE is not None) and (SCRTX_DET_FREQ_SHOP_TYPE != 0)):
        if ((SCRTX_DET_FREQ_SHOP_TYPE == 1) and (SCRTX_DET_FREQ_SHOP_VAL == 0)):
            return None
        elif ((SCRTX_DET_FREQ_SHOP_TYPE == 1) and (SCRTX_DET_FREQ_SHOP_VAL > 0)):
...
# MVP Changes - 2022-02-09
def saleQuantityMVP(SMA_RETL_MULT_UNIT, SCRTX_DET_FREQ_SHOP_VAL, SCRTX_DET_FREQ_SHOP_TYPE, SCRTX_HDR_DESC):
    if ((SCRTX_DET_FREQ_SHOP_TYPE is not None) and (SCRTX_DET_FREQ_SHOP_TYPE != 0)):
        if ((SCRTX_DET_FREQ_SHOP_TYPE == 1) and (SCRTX_DET_FREQ_SHOP_VAL == 0)):
            return None
        elif ((SCRTX_DET_FREQ_SHOP_TYPE == 1) and (SCRTX_DET_FREQ_SHOP_VAL > 0)):
...
def addNewNullColumn(itemMaster):
  ...
  itemMaster=itemMaster.withColumn("SMA_PRIME_UPC", when((col("ALTERNATE_UPC").isNull()), lit("N")).otherwise(lit("Y")).cast(StringType()))
...
#   itemMaster=itemMaster.withColumn("SALE_QUANTITY", lit(None).cast(StringType()))
  ...
  itemMaster=itemMaster.withColumn("SALE_PRICE", when((col("SALE_PRICE").isNull()), lit(None)).otherwise(lpad(formatZerosUDF(round(col("SALE_PRICE"), 2)),10,'0')).cast(StringType()))
  itemMaster=itemMaster.withColumn("SALE_QUANTITY", saleQuantityMVPUDF(col('SMA_RETL_MULT_UNIT'), col('SCRTX_DET_FREQ_SHOP_VAL'), col('SCRTX_DET_FREQ_SHOP_TYPE'), col('SCRTX_HDR_DESC')))
...
#     itemMasterOutputDf = itemMasterOutputDf.drop(col('PRD2STORE_BANNER_ID'))
    ...
    itemMasterOutputDf = itemMasterOutputDf.withColumn("SALE_PRICE", when((col("SALE_PRICE") == '0000000.00'), lit(None)).otherwise(col("SALE_PRICE")).cast(StringType()))
    itemMasterOutputDf = itemMasterOutputDf.withColumn("SALE_QUANTITY", when((col("SALE_QUANTITY") == '0'), lit(None)).otherwise(col("SALE_QUANTITY")).cast(StringType()))
    
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("EVENT_NAME", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("AD_WEEK", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("BRAND_DIVISION", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("MARKET_SEGMENT", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_PAGE", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_BLOCK", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_AD_PLACEMENT", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_AD_FORMAT", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_HEADLINE", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_SUBHEADLINE", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_AUSA_ITEM_NO", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ATTR_PRICE_TYPE_DESC", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("AD_COMMENTS", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("SID_COPYCARD", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("ADDITIONAL_SID", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("SOURCE_PROMOTIONID", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("CustomerlocationKey", lit(None).cast(StringType()))
#     itemMasterOutputDf=itemMasterOutputDf.withColumn("DEMANDTEC_PRISM_SID", lit(None).cast(StringType()))
...
#     itemMasterOutputDf.write.partitionBy('CHAIN_ID', 'SMA_DEST_STORE').mode('overwrite').format('parquet').save(itemMasterOutboundPath + "/" +"ItemMaster_Output")
    itemMasterOutputDf.write.partitionBy('CHAIN_ID', 'SMA_DEST_STORE').mode('overwrite').format('parquet').save("/mnt/centralized-price-promo/MVPTestinQA2")
...
#   unified_df = spark.sql("""(SELECT * FROM unified_minus_table) MINUS (SELECT *  FROM unified_minus_table WHERE PROMO_RTL_PRC is NULL and PROMOTIONAL_QUANTITY is NULL)""")
  unified_df = unified_df.withColumn("SALE_QUANTITY", when(unified_df.SCRTX_DET_FREQ_SHOP_TYPE == 0, None).when(unified_df.SCRTX_DET_FREQ_SHOP_TYPE == None, None).otherwise(unified_df.SALE_QUANTITY))
  unified_df = unified_df.withColumn("SALE_PRICE", when(unified_df.SCRTX_DET_FREQ_SHOP_TYPE == 0, None).when(unified_df.SCRTX_DET_FREQ_SHOP_TYPE == None, None).otherwise(unified_df.SALE_PRICE))
