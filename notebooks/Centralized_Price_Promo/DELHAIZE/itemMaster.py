...
#   unified_df = spark.sql("""(SELECT * FROM unified_minus_table) MINUS (SELECT *  FROM unified_minus_table WHERE PROMO_RTL_PRC is NULL and PROMOTIONAL_QUANTITY is NULL)""")
  unified_df = unified_df.withColumn("SALE_QUANTITY", when(unified_df.SCRTX_DET_FREQ_SHOP_TYPE == 0, None).otherwise(unified_df.SALE_QUANTITY))
  unified_df = unified_df.withColumn("SALE_PRICE", when(unified_df.SCRTX_DET_FREQ_SHOP_TYPE == 0, None).otherwise(unified_df.SALE_PRICE))
