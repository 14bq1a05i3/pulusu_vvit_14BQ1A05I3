...
def salePriceMVP(SCRTX_DET_FREQ_SHOP_TYPE, SCRTX_DET_FREQ_SHOP_VAL, SMA_RETL_MULT_UNIT, SMA_MULT_UNIT_RETL, SCRTX_HDR_DESC):
  ...
  if ((SCRTX_DET_FREQ_SHOP_TYPE is not None) and (SMA_MULT_UNIT_RETL is not None) and (SMA_RETL_MULT_UNIT is not None) and (SCRTX_DET_FREQ_SHOP_VAL is not None)):
    if int(SCRTX_DET_FREQ_SHOP_TYPE) == 1:
      return  (float(SMA_MULT_UNIT_RETL) - (float(SMA_RETL_MULT_UNIT) * float(SCRTX_DET_FREQ_SHOP_VAL)))
...
#   if SMA_RETL_MULT_UNIT is not None and SCRTX_HDR_DESC is not None:
#     if SCRTX_DET_FREQ_SHOP_TYPE == 1:
...
#   if SMA_MULT_UNIT_RETL is not None and SCRTX_HDR_DESC is not None:
#     if SCRTX_DET_FREQ_SHOP_TYPE == 1:
...
### Code should never update these 2 columns
...
if __name__ == "__main__":
  ...
  try:
    ...
    loggerAtt.info('Initial Item Master Count ' + str(itemMasterCount))
  except Exception as ex:
...
#   Since we are not updating the MULT_UNIT and UNIT_RTL, no need to include Price file
...
#   unified_df.createOrReplaceTempView("unified_minus_table1")
...
#   unified_df=unified_df.withColumn("SALE_PRICE", salePriceMVPUDF(col('SCRTX_DET_FREQ_SHOP_TYPE'), col('SCRTX_DET_FREQ_SHOP_VAL'), col('SMA_RETL_MULT_UNIT'), col('SMA_MULT_UNIT_RETL'), col('SCRTX_HDR_DESC')))
#   unified_df=unified_df.withColumn("SALE_PRICE", when((col("SALE_PRICE").isNull()), lit(None)).otherwise(lpad(formatZerosUDF(round(col("SALE_PRICE"), 2)),10,'0')).cast(StringType()))
  ...
  unified_df.createOrReplaceTempView("table1")
#   # 2022-01-21 Intraday
#   unified_df = unified_df.drop('SCRTX_DET_UNT_QTY')
#   unified_df = unified_df.drop('SCRTX_DET_RTL_PRC')
...
# MAGIC %sql SELECT SALE_PRICE, SALE_QUANTITY FROM table1 WHERE 
# MAGIC -- SMA_GTIN_NUM = '74390000500' AND SMA_DEST_STORE = 2255
# MAGIC SMA_GTIN_NUM = '3760157723050' AND SMA_DEST_STORE = 2255
