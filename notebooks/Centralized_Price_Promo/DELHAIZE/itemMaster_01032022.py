...
def retlMultUnit(SCRTX_DET_FREQ_SHOP_TYPE, SMA_RETL_MULT_UNIT, PRICE_MLT_Quantity, SCRTX_HDR_DESC):
  if PRICE_MLT_Quantity is not None and SCRTX_HDR_DESC is not None:
    if SCRTX_DET_FREQ_SHOP_TYPE == 1:
...
def multUnitRetl(SCRTX_DET_FREQ_SHOP_TYPE, SMA_MULT_UNIT_RETL, SCRTX_DET_RTL_PRC, SCRTX_HDR_DESC):
  if SCRTX_DET_RTL_PRC is not None and SCRTX_HDR_DESC is not None:
    if SCRTX_DET_FREQ_SHOP_TYPE == 1:
      return SCRTX_DET_RTL_PRC
    elif 'AD ON' in SCRTX_HDR_DESC:
      return SCRTX_DET_RTL_PRC
    else:
      return SMA_MULT_UNIT_RETL
...
#   itemMaster=itemMaster.withColumn("SALE_QUANTITY", lit(None).cast(StringType()))
  ...
  itemMaster=itemMaster.withColumn("SMA_MULT_UNIT_RETL", multUnitRetlUDF(col('SCRTX_DET_FREQ_SHOP_TYPE'),  col('SMA_MULT_UNIT_RETL'), col('SCRTX_DET_RTL_PRC'), col('SCRTX_HDR_DESC')))
...
#   findItemsNotInItemTemp(unified_df, itemTempEffDeltaPath, exceptionItemMainTempArchivalpath)
  ...
  unified_df1 = spark.sql("""select im.*, round(i.SCRTX_DET_RTL_PRC, 2) as SCRTX_DET_RTL_PRC from unified_table im JOIN ITEM_MAIN i ON i.RTX_STORE = im.SMA_DEST_STORE AND im.SMA_GTIN_NUM=i.RTX_UPC""")
  ...
  try:  
    ...
    if unified_df is not None:
      if unified_df.count() > 0:
        unified_df = addNewNullColumn(unified_df1)
  except Exception as ex:
