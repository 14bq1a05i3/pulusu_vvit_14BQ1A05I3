...
def BOFEOF(df):
  ...
  raw_df = df.filter(col('PRICE-Location').isNotNull())  
...
try:
  ABC(DeltaTableItemMainTempCreateCheck = 1)
  ...
  #spark.sql("OPTIMIZE ItemMainTemp")
  
except Exception as ex:
  ABC(DeltaTableItemMainTempCreateCheck = 0)
  loggerAtt.error(ex)
...
def itemMasterRecordsModified(itemTempEffDeltaPath, priceEffDeltaPath, PriceDeltaPath, Date, unifiedPriceFields):
  ...
  itemTempEffDelta = spark.sql('''select BANNER_ID,SMA_DEST_STORE,SMA_GTIN_NUM from delta.`{}`'''.format(itemTempEffDeltaPath))
  ...
  priceEffTemp = priceEffTemp.withColumn('SMA_LEGACY_ITEM', col('TPRX001_ITEM_NBR').cast(IntegerType()))
...
updateDeltaVersioning('Delhaize', 'price', PipelineID, Filepath, FileName)
