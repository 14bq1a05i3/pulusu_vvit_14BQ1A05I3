...
def abcFramework(headerFooterRecord, alternateUPC_duplicate_records, alternateUPC_raw_df):
    ...
    try:
        ...
        if (alternateUPC_raw_dfWithNoNull.groupBy('ALT_UPC').count().filter("count > 1").count()) > 0:
            ...
            alternateUPC_duplicate_records = alternateUPC_raw_dfWithNoNull.groupBy('ALT_UPC').count().filter(col('count') > 1)
            alternateUPC_duplicate_records = alternateUPC_duplicate_records.drop(alternateUPC_duplicate_records['count'])
            alternateUPC_duplicate_records = (alternateUPC_raw_dfWithNoNull.join(alternateUPC_duplicate_records,["ALT_UPC"], "leftsemi"))
            ...
            alternateUPCRecords = (alternateUPC_raw_dfWithNoNull.join(alternateUPC_duplicate_records,["ALT_UPC"], "leftanti"))  
            loggerAtt.info(f"Actual Record Count: {alternateUPCRecords}")
        else:
...
updateDeltaVersioning('Delhaize', 'altUPC', PipelineID, Filepath, FileName)
