...
try:
  ...
  ABC(DeltaTableCreateCheck = 0)
  loggerAtt.error(ex)
  err = ErrorReturn('Error', ex,'item temp deltaCreator')
  errJson = jsonpickle.encode(err)
  errJson = json.loads(errJson)
  dbutils.notebook.exit(Merge(ABCChecks,errJson)) 
...
updateDeltaVersioning('Delhaize', 'inventory', PipelineID, Filepath, FileName)
#updateDeltaVersioning('Delhaize', '', PipelineID, Filepath, FileName)
