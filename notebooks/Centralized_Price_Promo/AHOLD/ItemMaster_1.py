...
currentDate = datetime.datetime.now(timezone("America/Halifax")).date().isoformat()
...
def itemMasterOutputTransformation(itemMasterOutputDf):
  ...
  itemMasterOutputDf = itemMasterOutputDf.withColumn("SMA_RETL_MULT_UNIT", lpad(col("SMA_RETL_MULT_UNIT"),10,'0'))
  ...
  try:
    ...
    if processing_file == 'COD':
      ...
      currCoupon = spark.sql('''select  LOCATION, 
                                        ...
                                        todayUpdateItemMaster.LAST_UPDATE_TIMESTAMP as ItemMasterLAST_UPDATE_TIMESTAMP,
                                        couponRec.LAST_UPDATE_TIMESTAMP as couponRecLAST_UPDATE_TIMESTAMP
                                      from (select * from delta.`{}` where DATE(LAST_UPDATE_TIMESTAMP) between '{}' and '{}') as couponRec
                                      join delta.`{}` as todayUpdateItemMaster
                                      ON  todayUpdateItemMaster.SMA_LINK_HDR_COUPON = couponRec.COUPON_NO and 
                                          todayUpdateItemMaster.SMA_STORE = couponRec.LOCATION
                                      WHERE DATE(todayUpdateItemMaster.LAST_UPDATE_TIMESTAMP) < '{}' 
                                          or couponRec.LAST_UPDATE_TIMESTAMP > todayUpdateItemMaster.LAST_UPDATE_TIMESTAMP'''.format(couponDeltaPath, previousDate, currentDate, itemMasterDeltaPath, currentDate))
    ABC(currCouponCount='')
  except Exception as ex:
    loggerAtt.error(ex)
...
def itemMasterWrite(itemMasterDeltaPath,itemMasterOutboundPath):
  try:
    ...
    if processing_file == 'COD':
      itemMasterOutputDf = itemMasterOutputDf.filter((col('LAST_UPDATE_ID') == PipelineID))
