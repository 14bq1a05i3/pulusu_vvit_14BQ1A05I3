...
fullLoad = spark.read.format("csv").option("header", "true").option("inferSchema", "true").option("sep", '|').schema(itemRaw_schema).load("/mnt/ahold-centralized-price-promo/Fullitem/Inbound/RDS/2021/06/07/UnZipped/TEST.NQ.GM.SMA.DB.ITEM.LOAD.ZIP/TEST.NQ.GM.SMA.DB.ITEM.LOAD.txt")
...
# The applied options are for CSV files. For other file types, these will be ignored.
dailyLoad = spark.read.format("csv").option("header", "true").option("inferSchema", "true").option("sep", '|').schema(itemRaw_schema).load("/mnt/ahold-centralized-price-promo/Item/Inbound/RDS/2021/06/07/UnZipped/TEST.NQ.CI.SMA.DB.ITEM.MAINT.ZIP/TEST.NQ.CI.SMA.DB.ITEM.MAINT.txt")
...
# MAGIC %sql
# MAGIC 
# MAGIC select * from itemMasterAhold where SMA_CHG_TYPE='07'
...
# MAGIC %sql
# MAGIC 
# MAGIC Select * from PromotionLink where SMA_STORE= '0758' and SMA_PROMO_LINK_UPC in ('7566919675')