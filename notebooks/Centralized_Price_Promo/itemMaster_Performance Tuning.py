...
try:
 ...
 ## Performance  
  spark.sql("""OPTIMIZE itemMaster ZORDER BY (SMA_DEST_STORE)""")
...
try:
  ...
  CREATE TABLE IF NOT EXISTS Item_Main(
                RTX_STORE STRING,
                BANNER_ID STRING,
                COUPON_NO LONG,
                RTX_BATCH LONG,
              ...
              USING delta 
              LOCATION '{}' 
              PARTITIONED BY (RTX_STORE, SCRTX_HDR_ACT_DATE)
       
              """.format(itemMainDeltaPath))
  
  ## Performance  
  spark.sql("""OPTIMIZE Item_Main ZORDER BY (RTX_UPC)""")
...
try:
  ...
  CREATE  TABLE IF NOT EXISTS ItemMainEffTemp(
                SMA_DEST_STORE STRING,
                BANNER_ID STRING,
                SMA_LINK_HDR_COUPON STRING,
                SMA_BATCH_SERIAL_NBR INTEGER,
      ...
      """.format(itemTempEffDeltaPath))
  
  ## Performance  
  spark.sql("""OPTIMIZE ItemMainEffTemp ZORDER BY (SMA_BATCH_SERIAL_NBR)""")
...
try:
  ...
  CREATE  TABLE IF NOT EXISTS promoLink(
    ...
    ## Performance  
  spark.sql("""OPTIMIZE promoLink ZORDER BY (PROMO_COUPON_NO)""")
...
try:
  ...
  """.format(priceTempEffDeltaPath))
    ...
    ## Performance  
  spark.sql("""OPTIMIZE DApriceTemp ZORDER BY (PRICE_UPC)""")
...
try:
  ...
  """.format(inventoryTempEffDeltaPath))
    ...
    ## Performance  
  spark.sql("""OPTIMIZE InventoryTemp ZORDER BY (INVENTORY_UPC)""")
...
try:
    ...
    spark.sql("""CREATE TABLE IF NOT EXISTS prodToStoreTemp(
              ...
              LOCATION '{}'
              """.format(productToStoreTempEffDeltaPath))   
 ## Performance  
    spark.sql("""OPTIMIZE prodToStoreTemp ZORDER BY (PRD2STORE_UPC)""")
...
try:
  ...
  CREATE TABLE IF NOT EXISTS productTemp(
              ...
              LOCATION '{}' 
              """.format(productTempEffDeltaPath))
  
   ## Performance  
  spark.sql("""OPTIMIZE productTemp ZORDER BY (DSS_PRIMARY_ID)""")
...
try:
  ...
  """.format(couponDeltaPath))
    ...
    ## Performance  
  spark.sql("""OPTIMIZE CouponDelhaizeTable ZORDER BY (COUPON_NO)""")
...
def retlMultUnit(SCRTX_DET_FREQ_SHOP_TYPE, SMA_RETL_MULT_UNIT, PRICE_MLT_Quantity, SCRTX_HDR_DESC):
  if PRICE_MLT_Quantity is not None:
    if SCRTX_DET_FREQ_SHOP_TYPE == 1:
      return PRICE_MLT_Quantity
...
def multUnitRetl(SCRTX_DET_FREQ_SHOP_TYPE, SMA_MULT_UNIT_RETL, PRICE_UNIT_PRICE, SCRTX_HDR_DESC):
  if PRICE_UNIT_PRICE is not None:
    if SCRTX_DET_FREQ_SHOP_TYPE == 1:
      return PRICE_UNIT_PRICE
    elif 'AD ON' in SCRTX_HDR_DESC:
      return PRICE_UNIT_PRICE
    else:
      return SMA_MULT_UNIT_RETL
...
#   itemMaster=itemMaster.withColumn("SALE_QUANTITY", lit(None).cast(StringType()))
  ...
  itemMaster=itemMaster.withColumn("SMA_RETL_MULT_UNIT", retlMultUnitUDF(col('SCRTX_DET_FREQ_SHOP_TYPE'), col('SMA_RETL_MULT_UNIT'), col('PRICE_MLT_Quantity'), col('SCRTX_HDR_DESC')))
  
  itemMaster=itemMaster.withColumn("SMA_MULT_UNIT_RETL", multUnitRetlUDF(col('SCRTX_DET_FREQ_SHOP_TYPE'),  col('SMA_MULT_UNIT_RETL'), col('PRICE_UNIT_PRICE'), col('SCRTX_HDR_DESC')))
...
display(unified_df.select('SMA_LINK_UPC').distinct())
